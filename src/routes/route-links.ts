...
export const routesLinks: Array<RouteLinkType> = [
  ...
  },
  {
    ...
    handler: async (req: Request, res: Response) => {
      try {
        const appointment =
          await getAppointmentsRepository().getAppointmentByCalendarEventId(
            req.params.eventId
          );
...

  {
    ...
    handler: async (req: Request, res: Response) => {
      try {
        const appointment = await getAppointmentsRepository().getAppointment(
          req.params.id
        );
        if (!appointment) {
          return routeResponse(
    ...
    handler: async (req: Request, res: Response) => {
      try {
        ...
        return routeResponse(
          ...
          {
            has_error: true,
            message: "Failed to fetch calendar config",
            data: error?.message,
          },
          500
        );
      ...
      try {
        ...
        if (!accessToken || !event) {
          return routeResponse(
            ...
            {
              has_error: true,
              message: "Access token and event data are required",
              data: null,
            },
            400
          );
...

  {
    ...
    handler: async (req: Request, res: Response) => {
      try {
        ...
        const appointments =
          await getAppointmentsRepository().listTodayAppointments();
