...
export class StorageRepository implements IStorageRepository {
  ...
  async saveCallRecording(
    ...
    // Check if call history exists for this callSid
    ...
    if (existing) {
      ...
      if (duration !== undefined) {
        existing.callDuration = duration;
      }
      ...
      return await this.callHistoryRepo.save(callHistory);
    }
  }

  async updateCallRecordingUrl(
    callSid: string,
    recordingUrl: string
  ): Promise<CallHistory | null> {
    const callHistory = await this.callHistoryRepo.findOne({
      where: { callSid },
    });
    if (!callHistory) {
      return null;
  ...
  async updateCallStatus(
    ...
    if (!callHistory) {
      ...
      const newCallHistory = this.callHistoryRepo.create({
        callSid,
        status,
        prospectId,
      ...
      return await this.callHistoryRepo.save(newCallHistory);
    }
    callHistory.status = status;
    if (prospectId && !callHistory.prospectId) {
      callHistory.prospectId = prospectId;
    }
    return await this.callHistoryRepo.save(callHistory);
  }
